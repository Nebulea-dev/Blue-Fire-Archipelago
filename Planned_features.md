...
- @spencenox on the Archipelago server noticed that it's possible to access the first chest without a dash. It would be interesting to add a dashless option and make the dash an item. The first void gate is blocking, but it's possible to do it with either a spirit or another movement ability (spin attack, double jump, maybe even sprit ?)
- Add Spirits into the Archipelago logic/rules
- Add the DLC (Void gate region)
