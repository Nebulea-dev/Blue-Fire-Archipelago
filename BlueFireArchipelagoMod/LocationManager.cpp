...
bool LocationManager::OnLevelLoaded(UObject* Context, FFrame& Stack, void* RESULT_DECL)
{
	...
	if (PersistenceManager::saveDataFileExists())
	{
		...
		auto queuedItems = PersistenceManager::flushReceivedItems();
...
bool LocationManager::OnSpiritSlotBought(UObject* Context, FFrame& Stack, void* RESULT_DECL)
{
	...
	// Increment spiritSlotsPurchased in persistence and mark location as checked
	try
	{
		Json::Value data = PersistenceManager::readSaveData();
		int previousCount = data["spiritSlotsPurchased"].asInt();
		int newCount = previousCount + 1;

		// Check if we haven't exceeded max spirit slots
		if (newCount <= 8)
		{
			data["spiritSlotsPurchased"] = newCount;
			PersistenceManager::writeSaveData(data);
			Output::send<LogLevel::Verbose>(STR("Incremented spiritSlotsPurchased to {}\n"), newCount);

			// Mark the corresponding spirit slot shop location as checked
			// Spirit slot shop location IDs are sequential starting from a base ID
			// The base ID is generated by the Rust location generator
			int64_t spiritSlotLocationID = LocationsData::GetSpiritSlotShopBaseLocationID() + previousCount;
			LocationManager::SendOrQueueLocation(spiritSlotLocationID);
			Output::send<LogLevel::Verbose>(STR("Marked Spirit Slot Purchase {} (location {}) as checked\n"), newCount, spiritSlotLocationID);
		}
		else
		{
			Output::send<LogLevel::Error>(STR("Maximum spirit slots (8) already purchased\n"));
		}
	}
	catch (const std::exception&)
	{
		Output::send<LogLevel::Verbose>(STR("Exception while updating spiritSlotsPurchased\n"));
	}
...
{
	PersistenceManager::appendCheckedLocation(locationID);
}